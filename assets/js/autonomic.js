(()=>{
 const $=s=>document.querySelector(s),kind=document.body.dataset.lab,A=$('#agonist'),B=$('#antag');const motionPreference=window.matchMedia?window.matchMedia('(prefers-reduced-motion: reduce)'):null;let bound=false,motionPaused=!!motionPreference?.matches;const cells=[];
 ...
 function render(){
  ...
  const heart=$('#heart');if(heart){heart.setAttribute('data-motion',motionPaused?'paused':'running');heart.style.animationPlayState=motionPaused?'paused':'running';const motionButton=$('#motionToggle');if(motionButton){motionButton.setAttribute('aria-pressed',String(motionPaused));motionButton.textContent=motionPaused?'心臓の拍動を開始':'心臓の拍動を止める';}const motionStatus=$('#motionStatus');if(motionStatus)motionStatus.textContent=motionPaused?(motionPreference?.matches?'端末の「動きを減らす」設定に合わせて停止中です。ボタンで拍動を開始できます。':'拍動の模式表示は停止中です。'):'拍動の模式表示：動作中（実際の心拍数ではありません）';heart.style.setProperty('--beat-duration',(1.2/s.rate).toFixed(3)+'s');heart.style.setProperty('--beat-scale',String(kind==='16'?1.18+.12*s.response:1.18));}
 ...
 if(motionPreference?.addEventListener)motionPreference.addEventListener('change',()=>{motionPaused=motionPreference.matches;render();});
 A.oninput=render;B.oninput=render;$('#move').onclick=()=>{bound=true;render()};$('#reset').onclick=()=>{bound=false;motionPaused=!!motionPreference?.matches;if(motion){motion.setAttribute('aria-pressed','false');motion.textContent='模式図の動きを止める';}A.value='55';B.value='0';render()};render();
})();

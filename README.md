...
看護学生・薬理学初学者向けの静的HTML教材です。トップページと、現在利用できる38教材を収録しています。
...
現在のトップページはⅠ〜XVの新目次に沿っています。38本の教材を利用可能として案内し、未実装テーマは「予定」として区別しています。
...
- `nephron-model.js`：部位別のNa⁺・水再吸収、水透過性、輸送体、ADH、薬剤阻害、下流Na⁺送達を管理します。
- `nephron-view.js`：36・37が共用するネフロン図、再吸収方向、残量ゲージ、100個追跡表示です。
- LAB 36は正常な再吸収とADH ON/OFF、LAB 37は利尿薬の作用部位と輸送体BLOCKを扱います。
- 尿中Na⁺・尿量は相対方向であり、投与量から実測値を予測しません。

### LAB 38・39：電解質と浮腫

- LAB 38は36・37のネフロン図を再利用し、集合管主細胞のENaC、ROMK、Na⁺/K⁺-ATPase、アルドステロン作用を表示します。尿中Na⁺排泄と血清Na⁺濃度は別の指標です。
- LAB 39は細胞内液・血管内液・間質液を分け、毛細血管内圧、血漿膠質浸透圧、血管透過性、リンパ還流を独立して操作します。
- `electrolyte-model.js`／`collecting-duct-cell.js`と、`body-fluid-model.js`／`body-fluid-view.js`は後続LABから再利用できる独立部品です。
- 表示値は学習用の相対指標で、検査値、輸液量、利尿薬用量を予測しません。
